...
//###########################################################
var NAV_info, backNavPromiseResolve, backNavPromiseTimestamp, clearNavTree, createBackNavPromise, debugFrame, debugMode, displayState, escapeVoidState, getBackNavAction, getBrowserNavAction, getNavAction, historyStateChanged, isInternalBackNavAction, isValidHistoryState, isValidState, loadAppWithNavState, navReplace, navState, navigateBack, navigateTo, navigationLocked, navstatedisplay, pageloadAction, resolveInternalBackNav, rootState, storeNavInfo, updateAppWithNavState, updateNavState;
...
historyStateChanged = function(evnt) {
  var internalBackNav;
  if (!isValidHistoryState()) {
    return appLoaded(); //# we treat it as appload for now :-)
  }
  ...
  } else if (navState.base !== "VOID") {
    updateAppWithNavState(navState);
  }
};

//###########################################################
escapeVoidState = async function() {
...
isInternalBackNavAction = function(navAction) {
...
resolveInternalBackNav = function() {
...
isValidHistoryState = function() {
  return isValidState(history.state);
};

isValidState = function(state) {
  var i, idx, len, sKey, stateKeys, validKeys;
  if (state == null) {
    return false;
  }
  stateKeys = Object.keys(state);
  validKeys = Object.keys(rootState);
  if (stateKeys.length !== validKeys.length) {
    return false;
  }
  for (idx = i = 0, len = stateKeys.length; i < len; idx = ++i) {
    sKey = stateKeys[idx];
    if (sKey !== validKeys[idx]) {
      return false;
    }
  }
  return true;
